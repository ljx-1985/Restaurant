...
|-------------|-------------------------------------------------------------------------|----------------------|--------|------------|--------|-------------------|
| IT-APP-001  | 静态文件服务: 访问根路径 (`/`) 应返回状态码 200 和特定文本                | `GET /`              | 通过   | 2025-05-25 | AI     |                   |
| IT-APP-002  | 静态文件服务: 请求 `/index.html` 应返回状态码 200 和 HTML 内容类型        | `GET /index.html`    | 通过   | 2025-05-25 | AI     |                   |
| IT-APP-003  | `/test` 路由: 应返回状态码 200 和预期消息                                 | `GET /test`          | 通过   | 2025-05-25 | AI     |                   |
| IT-APP-004  | `/diagnose` 路由: 应返回状态码 200 和 JSON 格式的诊断信息                 | `GET /diagnose`      | 通过   | 2025-05-25 | AI     |                   |
| IT-APP-005  | `/recommend` 路由: 使用有效输入应返回状态码 200 和推荐菜品列表             | `POST /recommend`    | 通过   | 2025-05-25 | AI     |                   |
| IT-APP-006  | `/recommend` 路由: 使用无效输入（如缺少字段、类型错误）应返回400错误       | `POST /recommend`    | 通过   | 2025-05-25 | AI     | 包含多个子场景, 也覆盖preferences恶意内容检查 |
| IT-APP-009  | `/recommend` 路由: 使用有效的 `flavor_preferences` 应正确筛选和推荐菜品    | `POST /recommend`    | 通过   | 2025-05-25 | AI     | 在`app.test.js`中验证 |
| IT-APP-010  | `/recommend` 路由: 使用无效的 `flavor_preferences` (格式/类型错误) 应返回400 | `POST /recommend`    | 通过   | 2025-05-25 | AI     | 在`app.test.js`中验证 |
| IT-SEC-001  | `/recommend` 输入验证: 防止常见的XSS攻击 (如 `<script>`, `onerror`)       | `POST /recommend`    | 通过   | 2025-05-25 | AI     | 在`app.test.js`中验证 |
| IT-SEC-003  | HTTP响应头: 验证 `X-Content-Type-Options` 设置为 `nosniff`               | 所有响应             | 通过   | 2025-05-25 | AI     | 通过`helmet`设置, 在`app.test.js`中验证 |

## 4. 性能测试 (`app.js` API - 通过 `performance-test.yml` 和 Artillery 测试)

| 测试用例 ID | 测试描述                                                                 | API 端点/功能     | 状态   | 执行日期   | 执行人 | 备注/Artillery输出摘要 |
|-------------|--------------------------------------------------------------------------|-------------------|--------|------------|--------|------------------------|
| PERF-API-001| `/recommend` API 性能: 在高达100新用户/秒的负载下，响应时间保持毫秒级。 | `POST /recommend` | 通过   | 2025-05-26 | AI     | 平均响应:10ms, p95:24.8ms, p99:47.9ms, 所有请求成功 (12600/12600)。 |
